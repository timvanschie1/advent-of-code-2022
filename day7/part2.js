...
function part2(data) {
    ...
    const usedSpace = fileSystemTotalSizes['/'];
    const availableSpace = 70000000 - usedSpace;
    const minimumOfSpaceToBeDelete = neededSpace - availableSpace;

    let smallestSufficientSize = usedSpace;
    for (const dirPathString in fileSystemTotalSizes) {
        const fileSize = fileSystemTotalSizes[dirPathString];
        const notEnough = fileSize < minimumOfSpaceToBeDelete;
        if (notEnough) continue;
        smallestSufficientSize = Math.min(fileSize, smallestSufficientSize);
    }
